...
export function DriveTaskEngine({ tasks, roleObjective, donnaOverlay, pendingTaskId, error, onAction, onOpenModule }: {
  ...
  return <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_420px]">
    ...
    <section className="rounded-2xl border border-cyan-300/15 bg-white/[0.045] p-5 sm:p-6">
      ...
      {selected.action_config.completion_effects && <div className="mt-5 rounded-xl border border-cyan-300/15 bg-cyan-300/[0.04] p-4"><p className="text-xs uppercase tracking-wider text-cyan-200/70">When you complete this</p><ul className="mt-3 space-y-2 text-xs leading-5 text-white/55">{selected.action_config.completion_effects.document_name && <li>• Evidence is added to Transaction Room as {selected.action_config.completion_effects.document_name}.</li>}{Boolean(selected.action_config.completion_effects.notify_roles?.length) && <li>• The relevant downstream roles receive a DIN handoff.</li>}{selected.action_config.completion_effects.email_to_role && <li>• A delivery email is routed to {selected.action_config.completion_effects.email_to_role.replace(/[-_]/g, " ")}.</li>}<li>• Calendar and facilitator activity update automatically.</li></ul></div>}
