import { RealtimeAgent, RealtimeSession, type RealtimeAgentConfiguration } from '@openai/agents-realtime'
...
export async function startRealtimeSession(options: StartRealtimeOptions = {}) {
  ...
  // 2) Create a RealtimeAgent with tools
  ...
  const agent = new RealtimeAgent(agentOptions)

  // 3) Create session and auto-approve tool calls
  ...
  // 4) Connect via WebRTC (browser) with mic/speaker automatically managed
